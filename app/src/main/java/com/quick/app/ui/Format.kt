...
/** 温度类整数：设定温度 0x1A / 判断上下限 0x1B~0x1C，仪器按整数 ℃ 报值 */
...
/** 电压：寄存器 ×0.1 mV */
fun voltText(v: Double?): String = if (v == null) "--" else String.format(Locale.US, "%.1f mV", v)

/** 电阻：寄存器 ×0.1 Ω */
fun ohmText(v: Double?): String = if (v == null) "--" else String.format(Locale.US, "%.1f Ω", v)
